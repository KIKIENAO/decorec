...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.id_embedding_iter_list=[]
        self.modality_embedding_iter_list = []

        self.id_embedding_epoch_list = []
        self.modality_embedding_epoch_list = []
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None,lr=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            train_loss = self._train_epoch(train_data, epoch_idx, show_progress=show_progress,lr=lr)
            # if hasattr(self.model, "item_embedding"):
            #     self.id_embedding_epoch_list.append(self.model.item_embedding.weight.mean(dim=0).detach())
            #
            # if hasattr(self.model, "text_adaptor") and hasattr(self.model, "plm_embedding"):
            #     self.modality_embedding_epoch_list.append(
            #         self.model.text_adaptor(self.model.plm_embedding.weight).mean(dim=0).detach())
